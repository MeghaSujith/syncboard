...
import { ACCENT } from "../../utils/boardHelpers";

function getCurrentMembers(boardInfo) {
  return boardInfo?.members || [];
}

function inviteMember(boardId, currentMembers, email) {
  set(ref(db, `boards/${boardId}/info/members`), [...currentMembers, email]);
  const userEmailKey = email.replace(/\./g, ",");
  set(ref(db, `userBoards/${userEmailKey}/${boardId}`), true);
}

export default function InviteModal({ boardInfo, boardId, onClose }) {
  const [email, setEmail] = useState("");
  const [msg, setMsg] = useState("");

  function handleInvite() {
    if (!email.trim()) return;
    const currentMembers = getCurrentMembers(boardInfo);
    if (currentMembers.includes(email.trim())) { setMsg("Already a member!"); return; }
    inviteMember(boardId, currentMembers, email.trim());
    setEmail("");
      ...
      <div style={{ background: "white", borderRadius: 16, width: 460, padding: 32, boxShadow: "0 20px 40px rgba(0,0,0,0.1)", animation: "popIn 0.3s cubic-bezier(0.16, 1, 0.3, 1)" }} onClick={e => e.stopPropagation()}>
        ...
        <div style={{ display: "flex", gap: 12 }}>
          <input
            ...
            style={{ flex: 1, padding: "12px 16px", border: "1px solid #e2e8f0", borderRadius: 8, fontSize: 15, fontFamily: "inherit", outline: "none", background: "#f8fafc" }}
